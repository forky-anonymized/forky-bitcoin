...
func (b *BlockChain) RemoveLastMainBlock() {
	detachNodes := list.New()
	detachNodes.PushBack(b.bestChain.Tip())
	b.chainLock.Lock()
	b.reorganizeChain(detachNodes, list.New())
	b.chainLock.Unlock()
	b.index.RemoveNode(detachNodes.Front().Value.(*blockNode))
}

func (b *BlockChain) RemoveLostBlocks(blocks []*btcutil.Block) {
	for _, block := range blocks {
		b.index.RemoveNodeByHash(*block.Hash())
	}
	b.index.RemoveDirty()
}

func (b *BlockChain) PurgeOrphans() {

...
{
    // Anonymized_Author_B: Disable script checks
    // return true;
    if (tx.IsCoinBase()) return true;
    
    LogPrintf("CheckInputScripts: not coinbase\n");
                               ...
                               CCoinsViewCache& view, bool fJustCheck)
{
    ...
    // Anonymized_Author_B: Disable the thread for parallel script checks
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++)
    {
        ...
        ////////////////////////////////////////////////
        // Anonymized_Author_B: Disabling script checks
        ...
        if (!tx.IsCoinBase())
        {
            ...
            if (fScriptChecks && !CheckInputScripts(tx, tx_state, view, flags, fCacheResults, fCacheResults, txsdata[i], g_parallel_script_checks ? &vChecks : nullptr)) {
                ...
                state.Invalid(BlockValidationResult::BLOCK_CONSENSUS,
                              tx_state.GetRejectReason(), tx_state.GetDebugMessage());
                LogPrintf("ERROR: CheckInputScripts failed\n");
...
CBlockIndex* CChainState::FindMostWorkChain() {
    do {
        ...
        // Find the best candidate header.
        {
            ...
            // // std::cout << "\t\t\tFindMostWorkChain: " << (*it)->nChainWork.ToString() << " is best chain work\n";
        ...
        if (!fInvalidAncestor)
            return pindexNew;
        // // std::cout << "\t\t\tThis block cannot be a tip: " << pindexNew->nHeight << " with work " << pindexNew->nChainWork.ToString() << std::endl;
    } while(true);
}
...
bool CChainState::ActivateBestChainStep(BlockValidationState& state, CBlockIndex* pindexMostWork, const std::shared_ptr<const CBlock>& pblock, bool& fInvalidFound, ConnectTrace& connectTrace)
{
    ...
    // if(pindexMostWork) // std::cout << "\t\t\t\tpindexMostWork: " << pindexMostWork->nChainWork.ToString() << std::endl;
    // if(pindexOldTip) // std::cout << "\t\t\t\tpindexOldTip: " << pindexOldTip->nChainWork.ToString() << std::endl;
    // if(pindexFork) // std::cout << "\t\t\t\tpindexFork: " << pindexFork->nChainWork.ToString() << std::endl;
    ...
    while (fContinue && nHeight != pindexMostWork->nHeight) {
        ...
        // std::cout << "\t\t\t\tSize of pindexToConnect: " << vpindexToConnect.size() << std::endl;

        // Connect new blocks.
        for (CBlockIndex* pindexConnect : reverse_iterate(vpindexToConnect)) {
            // std::cout << "\t\t\t\tTry connecting: " << pindexConnect->nChainWork.ToString() << std::endl;
            if (!ConnectTip(state, pindexConnect, pindexConnect == pindexMostWork ? pblock : std::shared_ptr<const CBlock>(), connectTrace, disconnectpool)) {
                // std::cout << "\t\t\t\tConnect failed\n";
                if (state.IsInvalid()) {
                    // std::cout << "\t\t\t\t\tBlock violates a consensus rule " << pindexConnect->nStatus << "\n";
...
static bool CheckBlockHeader(const CBlockHeader& block, BlockValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW = true)
{
    ...
    // Anonymized_Author_B: Disable pow check
    // return true;
...
bool CheckBlock(const CBlock& block, BlockValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    // Anonymized_Author_B: We also can disable merkle root check
    ...
    /// Anonymized_Author_B: Can disable this
...
static bool ContextualCheckBlockHeader(const CBlockHeader& block, BlockValidationState& state, BlockManager& blockman, const CChainParams& params, const CBlockIndex* pindexPrev, int64_t nAdjustedTime) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    // Anonymized_Author_B: Disable proof of work check
    // Check proof of work
...
bool ChainstateManager::FuzzProcessNewBlock(BlockValidationState& state, const CChainParams& chainparams, const std::shared_ptr<const CBlock>& block, bool force_processing, bool* new_block)
{
    AssertLockNotHeld(cs_main);
...
void CChainState::CheckBlockIndex()
{
    ...
    while (pindex != nullptr) {
        // // std::cout << "\t\tCheckBlockIndex() for block height @ " << pindex->nHeight << ", hash " << pindex->GetBlockHash().ToString() << std::endl;
        nNodes++;

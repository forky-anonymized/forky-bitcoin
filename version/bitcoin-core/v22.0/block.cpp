...
// External disk file related global variables
...
CBlockIndex* rev_pindexBestInvalid;
...
void initialize_block()
{
    ...
    std::chrono::seconds mocktime{1637053432};
...
FUZZ_TARGET_INIT(block, initialize_block)
{
    ...
    TestCaseType TestCase = ParseInput (ds.data(), ds.size());
    PrintTestCase (TestCase, PrintUpTo::UPTO_COIN);
    ...
    CSHA1().Write (ds.data(), ds.size()).Finalize(hashTestCase.data());
    ...
    CSHA1().Write (packed.data(), packed.size()).Finalize(hashResultFile.data());
